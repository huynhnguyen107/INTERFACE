...
(
	...
	always @(posedge clk) begin
		if (!rst_n) begin
			...
			// Read calibration: from reg 0x88  to 0x8D 
				...
				r_start <= 0;	
				if (done)
					calib_o [{cnt_wait-1, 3'd0} +: 8] <= in_read_data;
			end
			...
			//Read raw temperature: from 0xFA  to 0xFC
				...
				r_start <= 0;
				if (done)
					raw_o [{cnt_raw-1, 3'd0} +: 8] <= in_read_data;
			end
	...
	.probe1(in_read_data), // input wire [7:0]  probe1 
	.probe2(valid_o), // input wire [0:0]  probe2 
	.probe3(calib_o), // input wire [47:0]  probe3 
	.probe4(raw_o) // input wire [23:0]  probe4
);
endmodule
